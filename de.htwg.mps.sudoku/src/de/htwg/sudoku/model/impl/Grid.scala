...
class Grid(cells: Vector[Cell]) extends GridTrait{
  ...
  def CountSolutions: Int={
    return solveAndCount
  }
  def solveAndCount: Int = {
    var Countsol: Int = 0
    var solutions: List[String] = List()
    for(i <- 0 until this.size; j <- 0 until this.size) {
		if(!(cell(i, j).isSet)) { 
	    	var AvailList: List[Int] = available(i, j).toList
			while(!AvailList.isEmpty) {
			   	var tmp: Grid = this
			   	var sol: Pair[Boolean, Grid] = tmp.set(i, j, AvailList.head).solve(0)
			   	AvailList = AvailList.tail
			   	if(sol._1 && !(solutions.contains((sol._2).toString))) {
			   	  Countsol += 1
			   	  solutions = sol._2.toString :: solutions
			   	}
			}
		}
    }
