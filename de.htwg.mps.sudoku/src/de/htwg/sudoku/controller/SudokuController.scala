...
class SudokuController(var grid: Grid) extends Publisher {
  ...
  def getSymGrid{
    //implementation of symmetric sudoku
    val (sucess, g) = grid.solve
    grid = g
    //remove symmetric
    do{
    	removeSym(rand, rand)
    }while(grid.CountSolutions <= 1) 
    
  }
  def rand:Int={
    val ran = new java.util.Random()
    val r = ran.nextInt(grid.size)
    r
  }
  def removeSym(col: Int, row: Int) {
    grid = grid.set(row, col, 0)
    grid = grid.set(grid.size-row-1, grid.size-col-1, 0)
  }
}